...
> **Status: in active development.** The scanner collects from live AWS accounts and runs three CIS checks end to end, with 39 tests running in CI. Remediation, the findings API, and the dashboard are not built. Sections below are labeled **Built** or **Planned**; the [Roadmap](#roadmap) tracks progress.
