...
> **Status: in active development.** The scanner collects from live AWS accounts and runs the CIS checks end to end, with 39 tests running in CI. Remediation, the findings API, and the dashboard are built and functional. Sections below are labeled **Built** or **Planned**; the [Roadmap](#roadmap) tracks progress.
...
    subgraph PLATFORM["Findings platform — built"]
...
**Solid boxes are implemented.** Remediation, the findings store, API, and dashboard are built. Slack alerting remains planned.
...
* **Suppression guardrails** *(Built).* Suppression requires suppressed_by, justification, and expires_at, preventing undocumented and indefinite suppression.

* **Read-time finding status** *(Built).* Finding status is computed when findings are read, so expired suppressions naturally stop affecting reported status without rewriting historical rows.
...
* **Tooling comparison** *(Built).* Scanner output benchmarked against Prowler and ScoutSuite to document coverage gaps honestly.
...
├── api/                    # FastAPI findings service (built)

├── dashboard/              # Next.js findings UI (built)
...
[`docs/architecture.md`](docs/architecture.md) records sixteen design decisions with their reasoning, rejected alternatives, and costs. The ones that shape everything else:
...
The remaining open questions are primarily around the findings platform rather than the scanner’s core detection model:

API authentication and authorization. The API currently has no authentication. Before external or multi-user deployment, the API needs an identity model and authorization rules governing who can view, suppress, unsuppress, or otherwise modify findings.
...
Trend data granularity. The current trend view has only one data point, so it demonstrates the visualization path but does not yet provide a meaningful historical trend. A scheduled scan/history strategy is needed before the trend view represents useful longitudinal data.
...
* [x] FastAPI findings API + Next.js dashboard (Unit 10)
