...
> **Status: in active development.** The scanner collects from live AWS accounts and runs three CIS checks end to end. Remediation, the findings API, and the dashboard are not built. Sections below are labeled **Built** or **Planned**; the [Roadmap](#roadmap) tracks progress.
...
| 2.14        | IAM policy allowing full `*:*` admin privileges is attached | **Built** | Critical            |
...
**2.14** identifies attached IAM policies that grant `Action: "*"` on `Resource: "*"`. Service-specific wildcards such as `s3:*` do not qualify as full administrative access, and unattached policies are ignored because the control is concerned with policies that actually grant privileges.
...
* **Only three of six planned checks are built.**
